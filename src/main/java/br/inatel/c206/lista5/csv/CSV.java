package br.inatel.c206.lista5.csv;

import br.inatel.c206.lista5.funcionario.Funcionario;
...
public class CSV {
    ...
    private static ArrayList<Funcionario> FilterFuncs(ArrayList<Funcionario> funcs){
        funcs.removeIf(func -> func.getFilhos() <= 0);
        return funcs;
    }
    ...
    public static void generateCSVFuncs(Path arquivo){
        ...
        try{
            for(Funcionario f : funcs){
                frase = f.getId()+","+f.getFilhos()+","+f.getSalario()+"\n";
                try {
                    //Java11
